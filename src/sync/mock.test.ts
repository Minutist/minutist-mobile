...
import type { Meeting, SyncStatus } from './types';
...
describe('MockSyncClient.enrolmentState', () => {
  it('defaults to enrolled so tests that do not care can sync', async () => {
    await expect(client.enrolmentState()).resolves.toBe('enrolled');
  });

  it('reports each state the UI distinguishes', async () => {
    for (const state of ['unknown', 'enrolled', 'awaitingConfirmation', 'fault'] as const) {
      client.setEnrolmentState(state);
      await expect(client.enrolmentState()).resolves.toBe(state);
    }
  });
});
...
describe('MockSyncClient.onStatus', () => {
  it('unsubscribe prevents further callbacks', async () => {
    const captured = new MockSyncClient({ noFixtures: true });
    const payload = { title: 'rec', startedAt: Date.now(), durationMs: 1_000, notes: '' };
    const a = await captured.saveCaptured(payload);
    const b = await captured.saveCaptured(payload);
    const statuses: SyncStatus[] = [];
    const unsub = captured.onStatus((s) => statuses.push(s));
    await captured.syncMeeting(a);
    const seen = statuses.length;
    unsub();
    await captured.syncMeeting(b);
    // Nothing further arrives once unsubscribed.
