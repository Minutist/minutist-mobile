...
`nw_path_is_ultra_constrained`. Confirmed end to end on the ALPN `/2` rebuild —
the symbol is absent from the library and the app launches and renders on the
iPhone 6s, where the earlier build aborted in dyld before any code ran.

One trap in the device build chain. `ios/App/App/capacitor.config.json` and
`ios/App/App/public/` are generated by `cap sync ios` and are gitignored, which
is exactly what makes an `rsync --delete --filter=':- .gitignore'` to the Mac
strip them — the build then fails on a missing config rather than anything real.
Any rsync to the Mac must be followed by `npm run build && npx cap sync ios`
before `xcodebuild`, which is what `scripts/ios-build-on-mac.sh` already does.
